...
module EPDQ
  class Request
    ...
    # the EPDQ.sha_in.
    def shasign
      calculator = EPDQ::ShaCalculator.new(full_parameters, @account.sha_in, @account.sha_type)
    ...
    def request_url
      @account.test_mode ? TEST_URL : LIVE_URL
    end

    private

    def full_parameters
      @parameters.merge({ :pspid => @account.pspid })
    end
